...
- Independent validation sources (NEVER use for training, feature selection or tuning — test only): TEMIS/KNMI daily noon UVI (clear-sky and cloudy), NASA OMI OMUVB daily overpass (UVI + irradiance at 305/310/324/380 nm; fetch with `earthaccess`, needs an Earthdata login in `.env`). Report error separately per source.
  - Validation split: TEMIS/OMI data from **2023 only** may be used, and only in `source_code/notebooks/02_source_selection.ipynb`, to choose the training target source (Open-Meteo vs NASA POWER uv_index) — done, see "Ground truth". TEMIS/OMI **2025 is the held-out test set** (download OMI 2025 only on day 10; do not load, plot, print statistics of or otherwise inspect it before day 10 — printing a row count is allowed). **TEMIS/OMI 2024 is not used at all**, because it overlaps the model dev year. Code must read validation data through `src.fetch_validation.load_validation()`, which returns 2023 by default and only 2025 with `split="test"`.
- **Data split for modelling (decided day 6), `src/splits.py`:**
  - **Train = 2023, Dev = 2024.** Use them for model comparison, Optuna and TimeSeriesSplit on days 6–9. TimeSeriesSplit folds must stay inside 2023–2024.
  - **Test = 2025.** Do not load, look at or compute metrics on it (NASA POWER targets or TEMIS/OMI) before day 10. Model code reads `train.parquet` only through `src.splits.load_model_data()`, which filters 2025 out at read time. `chronological_split()` asserts that no 2025 row is present.
  - **Day 10:** refit the chosen model on 2023–2024, then evaluate it on 2025 **once**, against NASA POWER and TEMIS/OMI 2025. Report the test set separately from the dev results.
...
- Ground truth: **NASA POWER** hourly `ALLSKY_SFC_UV_INDEX`, `ALLSKY_SFC_UVA`, `ALLSKY_SFC_UVB` (satellite/model-based). Decided on day 2 in `02_source_selection.ipynb` with a rule declared before looking at results (primary metric: MAE vs OMI all-sky noon UVI, 2023; pick NASA POWER only if its MAE is lower by more than 0.3 UVI, otherwise Open-Meteo). Result: NASA POWER 1.143 vs Open-Meteo 2.428 (n=275); Open-Meteo saturates near UVI 9.3 and its clear-sky UVI is ~3.5 below TEMIS. State clearly in docs/report that no physical UV instrument was used.
  - **Model features must come only from Open-Meteo (weather / air quality, available as forecast at run time) or be computed from time and location** (solar geometry, hour/month terms). NEVER use NASA POWER columns as features (CLOUD_AMT, ALLSKY/CLRSKY_SFC_SW_DWN, any clear-sky index built from them, TO3, or the UV targets): (1) they are not available at run time, (2) they are target leakage — same satellite product as the targets and nearly equal to the CMF itself. CMF denominators (`uvi_clear`, `uva_clear`, `uvb_clear`) and `ozone_clim_du` are not features either. The authoritative feature list is `FEATURES` in `src/features.py` / `source_code/models/dataset_spec_v1.json`; later sensors (phone lux, sky-image CNN) are added there explicitly.
...
- Ozone input O3 (total column, DU): the monthly climatology `source_code/models/ozone_climatology_v2.json` (built from NASA POWER TO3 **2023–2024 only**, Asia/Bangkok months; v1 also used the test year 2025 and was removed on day 6), read with `src.physics.ozone_climatology()`. Use it by default for BOTH training targets and the app/API (train–serve consistency), and to fill TO3 gaps. Do not use a constant 300 DU (−15 % UVI bias in the tropics). Surface ozone from Open-Meteo (µg/m³) is NOT column ozone.
